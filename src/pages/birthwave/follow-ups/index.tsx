...
const Group = ({ title, tone, leads }: GroupProps) => (
  <Box sx={{ bgcolor: '#FFFFFF', border: '1px solid', borderColor: CARD_BORDER, borderRadius: '14px', overflow: 'hidden', mb: 2.5 }}>
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3, lg: 4 } }}>
      ...
      {isLoading ? (
        ...
        <>
          <Group title="Overdue" tone={{ bg: '#FEF2F2', fg: '#EF4444' }} leads={overdue} />
          <Group title="Today" tone={{ bg: '#FFFBEB', fg: '#F59E0B' }} leads={today} />
          <Group title="Upcoming" tone={{ bg: '#EFF6FF', fg: '#2563EB' }} leads={upcoming} />
        </>
      )}
    </Box>
  );
