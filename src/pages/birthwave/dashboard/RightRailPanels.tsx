...
const FollowUpBadge = ({ dueValue }: { dueValue: string }) => {
  ...
  if (diff < 0) return <Chip label="Overdue" size="small" sx={{ bgcolor: '#FEF2F2', color: '#EF4444', fontWeight: 700, fontSize: '0.65rem', height: 20 }} />;
  if (diff === 0) return <Chip label="Today" size="small" sx={{ bgcolor: '#FFFBEB', color: '#F59E0B', fontWeight: 700, fontSize: '0.65rem', height: 20 }} />;
  return <Chip label={formatDate(dueValue)} size="small" sx={{ bgcolor: '#F1F5F9', color: TEXT_MUTED, fontWeight: 700, fontSize: '0.65rem', height: 20 }} />;
};
        ...
        <Stack direction="column" spacing={1.5}>
          {followUps.map((lead) => (
            <Stack
              ...
              sx={{ cursor: 'pointer', borderRadius: '8px', mx: -1, px: 1, py: 0.25, '&:hover': { bgcolor: 'rgba(15,23,42,0.03)' }, '&:focus-visible': { outline: '2px solid #29AF81', outlineOffset: 1 } }}
            >
      ...
      {loading ? (
        ...
        <Stack direction="column" spacing={0}>
          {schedule.map((appt, index) => {
            const statusColor = APPOINTMENT_STATUS_COLORS[appt.status] || { bg: '#F1F5F9', fg: TEXT_MUTED };
            return (
              <Stack
                key={appt.id}
                direction="row"
                ...
                sx={{
                  ...
                  '&:hover': appt.lead ? { bgcolor: 'rgba(15,23,42,0.03)' } : undefined,
