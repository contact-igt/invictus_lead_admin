...
const LeadSourcesDonut = ({ sources, totalLeads, loading, onSourceClick }: LeadSourcesDonutProps) => {
  ...
  const sliceBorder = mode === 'dark' ? '#0D1410' : '#fff';
  ...
  return (
    <Box sx={{ ...cardSx, height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {loading ? (
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 2, sm: 1.25 }} alignItems="center" flex={1}>
          <Box
            sx={{
              position: 'relative',
              width: { xs: 160, sm: 140 },
          ...
          <Stack
            direction="column"
            spacing={0.75}
            sx={{ width: { xs: '100%', sm: 0 }, minWidth: 0, flex: '1 1 0' }}
          >
            {sources.map((s, i) => (
              <Stack
                key={s.source}
                ...
                sx={{
                  ...
                  px: 0.5,
                  mx: -0.5,
                  '&:hover': onSourceClick ? { bgcolor: 'var(--bw-hover)' } : undefined,
              ...
              >
                ...
                />
                <Typography noWrap sx={{ fontSize: '0.8rem', color: TEXT_DARK, minWidth: 0, flexGrow: 1 }}>
                  {LEAD_SOURCE_LABELS[s.source] || s.source}
                </Typography>
                <Typography sx={{ fontSize: '0.78rem', fontWeight: 700, color: TEXT_DARK, flexShrink: 0 }}>
                  {s.count}
                </Typography>
                <Typography sx={{ fontSize: '0.72rem', color: TEXT_MUTED, width: 36, textAlign: 'right', flexShrink: 0 }}>
                  {s.percentage}%
                </Typography>
              </Stack>
            ))}
