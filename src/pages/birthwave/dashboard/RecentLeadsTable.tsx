...
const RecentLeadsTable = ({ clientKey, leads, loading }: RecentLeadsTableProps) => {
  ...
  const hrefFor = (lead: BirthwaveLead) =>
    lead.kind === 'website' && lead.source_key
      ? `${buildClientPortalPath(clientKey, 'leads')}?view=${lead.source_key}`
      : buildClientPortalPath(clientKey, `leads/${lead.id}`);
          ...
          <Box component="tbody">
            {leads.map((lead) => {
              ...
              return (
                <Box
                  component="tr"
                  key={`${lead.kind || 'crm'}-${lead.id}`}
                  onClick={() => navigate(hrefFor(lead))}
                  role="button"
                  tabIndex={0}
