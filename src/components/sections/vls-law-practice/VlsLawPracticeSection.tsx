...
import {
  useCreateVlsLawPracticeRegistration,
  useDeleteVlsLawPracticeRegistration,
  useUpdateVlsLawPracticeRegistration,
  useVlsLawPracticeRegistration,
  useVlsLawPracticeRegistrations,
  useVlsLawPracticeSummary,
} from 'hooks/useVlsLawPracticeQuery';
import { useAuth } from 'redux/selectors/auth/authSelector';
import { VLS_LAW_PRACTICE_PAGE_NAME_OPTIONS } from 'schemas/vlsLawPracticeSchema';
import type { VlsLawPracticeFormValues } from 'schemas/vlsLawPracticeSchema';
import { exportVlsLawPracticeRegistrations } from 'services/vlsLawPractice.service';
import type {
  VlsLawPracticeExportFormat,
  VlsLawPracticeListParams,
  VlsLawPracticeRegistration,
  VlsLawPracticeSummary,
} from 'types/vlsLawPractice';
import { resolveClientModuleKey } from 'utils/clientModuleResolver';
import VlsLawPracticeDeleteDialog from './VlsLawPracticeDeleteDialog';
import VlsLawPracticeFormDrawer, {
  VlsLawPracticeDrawerMode,
} from './VlsLawPracticeFormDrawer';
import VlsLawPracticeTable from './VlsLawPracticeTable';
import {
  VLS_LAW_PRACTICE_COLOR,
  VLS_LAW_PRACTICE_PAGE_NAME_LABELS,
  VLS_LAW_PRACTICE_PAYMENT_STATUS_OPTIONS,
  cleanVlsLawPracticePayload,
  extractDownloadFilename,
  getVlsLawPracticeErrorMessage,
  getVlsLawPracticeExportErrorMessage,
  getVlsLawPracticeExportFallbackName,
  hasVlsLawPracticeFilters,
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={1.5}>
        ...
        <Box
          sx={{
            ...
            display: 'grid',
            placeItems: 'center',
            bgcolor: alpha(VLS_LAW_PRACTICE_COLOR, 0.1),
            color: VLS_LAW_PRACTICE_COLOR,
            flexShrink: 0,
          }}
        >
...
const EMPTY_SUMMARY: VlsLawPracticeSummary = {
...
const VlsLawPracticeSection = () => {
  ...
  const [drawerMode, setDrawerMode] = useState<VlsLawPracticeDrawerMode>('create');
  const [selectedRegistration, setSelectedRegistration] =
    useState<VlsLawPracticeRegistration | null>(null);
  ...
  const params: VlsLawPracticeListParams = {
    page,
    limit,
  ...
  const registrationsQuery = useVlsLawPracticeRegistrations(clientKey, params, isVlsModule);
  const summaryQuery = useVlsLawPracticeSummary(clientKey, params, isVlsModule);
  const detailQuery = useVlsLawPracticeRegistration(
    clientKey,
    selectedRegistration?.id ?? null,
    isVlsModule && drawerOpen && drawerMode === 'view',
  );
  const createMutation = useCreateVlsLawPracticeRegistration(clientKey);
  const updateMutation = useUpdateVlsLawPracticeRegistration(clientKey);
  const deleteMutation = useDeleteVlsLawPracticeRegistration(clientKey);
  ...
  const openDrawer = (
    ...
    registration: VlsLawPracticeRegistration | null = null,
  ) => {
  ...
  const handleSubmit = (values: VlsLawPracticeFormValues) => {
    const payload = cleanVlsLawPracticePayload(values);
  ...
  const handleExport = async (format: VlsLawPracticeExportFormat) => {
    handleCloseExportMenu();
    setIsExporting(true);
    try {
      const response = await exportVlsLawPracticeRegistrations(
        format,
        params,
        superAdminClientKey,
      );
      ...
      const fileName = extractDownloadFilename(
        ...
        getVlsLawPracticeExportFallbackName(format),
      );
      saveAs(new Blob([response.data], { type: contentType }), fileName);
      enqueueSnackbar(
        `Law Practice enrollments exported as ${format.toUpperCase()} successfully.`,
        { variant: 'success' },
      );
    } catch (error) {
      enqueueSnackbar(getVlsLawPracticeExportErrorMessage(error), { variant: 'error' });
    } finally {
  ...
  const hasFilters = hasVlsLawPracticeFilters(params);
    ...
    >
      <Paper
        ...
        sx={{
          ...
          p: { xs: 2.25, md: 3 },
          borderRadius: 3,
          bgcolor: alpha(VLS_LAW_PRACTICE_COLOR, 0.035),
          borderColor: alpha(VLS_LAW_PRACTICE_COLOR, 0.18),
        }}
      >
        <Stack
        ...
        >
          <Stack direction="row" alignItems="center" spacing={1.75} sx={{ minWidth: 0, flex: 1 }}>
            <Box
              sx={{
                ...
                placeItems: 'center',
                color: 'common.white',
                bgcolor: VLS_LAW_PRACTICE_COLOR,
              }}
            >
            ...
            <Box sx={{ minWidth: 0 }}>
              ...
              >
                Law Practice Enrollments
              </Typography>
              <Typography variant="body2" color="text.secondary" mt={0.35}>
                Manage VLS Law practice enrollment and payment data
              </Typography>
            </Box>
          </Stack>
          <Stack direction="row" spacing={1.25} flexWrap="wrap" justifyContent="flex-end">
            <Button
              ...
              sx={{
                borderColor: alpha(VLS_LAW_PRACTICE_COLOR, 0.28),
                color: VLS_LAW_PRACTICE_COLOR,
                flexShrink: 0,
                '&:hover': { borderColor: VLS_LAW_PRACTICE_COLOR, bgcolor: alpha(VLS_LAW_PRACTICE_COLOR, 0.04) },
              }}
            >
            ...
            <Button
              ...
              sx={{
                bgcolor: VLS_LAW_PRACTICE_COLOR,
                flexShrink: 0,
                '&:hover': { bgcolor: alpha(VLS_LAW_PRACTICE_COLOR, 0.88) },
              }}
            >
              Add Enrollment
            </Button>
        ...
        <Box sx={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
          ...
          >
            {getVlsLawPracticeErrorMessage(
              ...
              'Unable to load Law Practice enrollments.',
            )}
          </Alert>
        </Box>
      ) : (
