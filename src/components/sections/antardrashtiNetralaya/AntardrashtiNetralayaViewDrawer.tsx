...
const AntardrashtiNetralayaViewDrawer = ({ open, lead, isLoading, onClose }: Props) => {
    ...
    <Drawer anchor="right" open={open} onClose={isLoading ? undefined : onClose} PaperProps={{ sx: { width: { xs: '100vw', sm: 580 }, maxWidth: '100vw' } }}>
      <Box sx={{ px: { xs: 2, sm: 3 }, py: 2.5 }}>
        <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
          <Box>
            <Typography variant="h5" fontWeight={750}>Antardrashti Netralaya Lead</Typography>
      ...
      <Box sx={{ px: { xs: 2, sm: 3 }, py: 3 }}>
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 3 }}>
          ...
          {item('Service', lead?.service)}
          {item('IP Address', lead?.ip_address, true)}
          {item('UTM Source', lead?.utm_source)}
        </Box>
        <Divider sx={{ my: 3 }} />
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 3 }}>
          {item('Created At', formatAntardrashtiDateTime(lead?.created_at))}
          {item('Updated At', formatAntardrashtiDateTime(lead?.updated_at))}
        </Box>
      </Box>
    </Drawer>
  );
...
export default AntardrashtiNetralayaViewDrawer;
