...
import { formatAntardrashtiDateTime } from './antardrashtiNetralayaUtils';

interface AntardrashtiNetralayaTableProps {
  rows: AntardrashtiNetralayaLead[];
  page: number;
  limit: number;
  total: number;
  isLoading: boolean;
  ...
  onView: (lead: AntardrashtiNetralayaLead) => void;
  onEdit: (lead: AntardrashtiNetralayaLead) => void;
  onDelete: (lead: AntardrashtiNetralayaLead) => void;
}

interface AntardrashtiNetralayaTableRow extends AntardrashtiNetralayaLead {
  serial_number: number;
}

const AntardrashtiNetralayaTable = ({
  rows,
  page,
  limit,
  total,
  ...
  onEdit,
  onDelete,
}: AntardrashtiNetralayaTableProps) => {
  const tableRows: AntardrashtiNetralayaTableRow[] = rows.map((row, index) => ({
    ...row,
    serial_number: (page - 1) * limit + index + 1,
  }));

  const columns: GridColDef<AntardrashtiNetralayaTableRow>[] = [
    {
      field: 'serial_number',
      headerName: 'S.No',
      width: 72,
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Typography variant="body2" color="text.secondary">
          {formatAntardrashtiDateTime(params.row.created_at)}
        </Typography>
      ),
    },
    {
    ...
    return (
      ...
      >
        ...
        <Typography variant="h6" mt={1.5}>
          {hasFilters ? 'No leads match the selected filters.' : 'No Antardrashti Netralaya leads yet'}
        </Typography>
        {!hasFilters && (
...
export default AntardrashtiNetralayaTable;
