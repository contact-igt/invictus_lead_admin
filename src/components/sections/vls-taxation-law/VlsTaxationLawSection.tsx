...
import {
  useCreateVlsTaxationLawRegistration,
  useDeleteVlsTaxationLawRegistration,
  useUpdateVlsTaxationLawRegistration,
  useVlsTaxationLawRegistration,
  useVlsTaxationLawRegistrations,
  useVlsTaxationLawSummary,
} from 'hooks/useVlsTaxationLawQuery';
import { useAuth } from 'redux/selectors/auth/authSelector';
import type { VlsTaxationLawFormValues } from 'schemas/vlsTaxationLawSchema';
import { exportVlsTaxationLawRegistrations } from 'services/vlsTaxationLaw.service';
import type {
  VlsTaxationLawExportFormat,
  VlsTaxationLawListParams,
  VlsTaxationLawRegistration,
  VlsTaxationLawSummary,
} from 'types/vlsTaxationLaw';
import { resolveClientModuleKey } from 'utils/clientModuleResolver';
import VlsTaxationLawDeleteDialog from './VlsTaxationLawDeleteDialog';
import VlsTaxationLawFormDrawer, {
  VlsTaxationLawDrawerMode,
} from './VlsTaxationLawFormDrawer';
import VlsTaxationLawTable from './VlsTaxationLawTable';
import {
  VLS_TAXATION_LAW_COLOR,
  VLS_TAXATION_LAW_PAYMENT_STATUS_OPTIONS,
  cleanVlsTaxationLawPayload,
  extractDownloadFilename,
  getVlsTaxationLawErrorMessage,
  getVlsTaxationLawExportErrorMessage,
  getVlsTaxationLawExportFallbackName,
  hasVlsTaxationLawFilters,
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={1.5}>
        ...
        <Box
          sx={{
            ...
            display: 'grid',
            placeItems: 'center',
            bgcolor: alpha(VLS_TAXATION_LAW_COLOR, 0.1),
            color: VLS_TAXATION_LAW_COLOR,
            flexShrink: 0,
          }}
        >
...
const EMPTY_SUMMARY: VlsTaxationLawSummary = {
...
const VlsTaxationLawSection = () => {
  ...
  const [drawerMode, setDrawerMode] = useState<VlsTaxationLawDrawerMode>('create');
  const [selectedRegistration, setSelectedRegistration] =
    useState<VlsTaxationLawRegistration | null>(null);
  ...
  const params: VlsTaxationLawListParams = {
    page,
    limit,
  ...
  const registrationsQuery = useVlsTaxationLawRegistrations(clientKey, params, isVlsModule);
  const summaryQuery = useVlsTaxationLawSummary(clientKey, params, isVlsModule);
  const detailQuery = useVlsTaxationLawRegistration(
    clientKey,
    selectedRegistration?.id ?? null,
    isVlsModule && drawerOpen && drawerMode === 'view',
  );
  const createMutation = useCreateVlsTaxationLawRegistration(clientKey);
  const updateMutation = useUpdateVlsTaxationLawRegistration(clientKey);
  const deleteMutation = useDeleteVlsTaxationLawRegistration(clientKey);
  ...
  const openDrawer = (
    ...
    registration: VlsTaxationLawRegistration | null = null,
  ) => {
  ...
  const handleSubmit = (values: VlsTaxationLawFormValues) => {
    const payload = cleanVlsTaxationLawPayload(values);
  ...
  const handleExport = async (format: VlsTaxationLawExportFormat) => {
    handleCloseExportMenu();
    setIsExporting(true);
    try {
      const response = await exportVlsTaxationLawRegistrations(
        format,
        params,
        superAdminClientKey,
      );
      ...
      const fileName = extractDownloadFilename(
        ...
        getVlsTaxationLawExportFallbackName(format),
      );
      saveAs(new Blob([response.data], { type: contentType }), fileName);
      enqueueSnackbar(
        `Taxation Law registrations exported as ${format.toUpperCase()} successfully.`,
        { variant: 'success' },
      );
    } catch (error) {
      enqueueSnackbar(getVlsTaxationLawExportErrorMessage(error), { variant: 'error' });
    } finally {
  ...
  const hasFilters = hasVlsTaxationLawFilters(params);
    ...
    >
      <Paper
        ...
        sx={{
          ...
          p: { xs: 2.25, md: 3 },
          borderRadius: 3,
          bgcolor: alpha(VLS_TAXATION_LAW_COLOR, 0.035),
          borderColor: alpha(VLS_TAXATION_LAW_COLOR, 0.18),
        }}
      >
        <Stack
        ...
        >
          <Stack direction="row" alignItems="center" spacing={1.75} sx={{ minWidth: 0, flex: 1 }}>
            <Box
              sx={{
                ...
                placeItems: 'center',
                color: 'common.white',
                bgcolor: VLS_TAXATION_LAW_COLOR,
              }}
            >
            ...
            <Box sx={{ minWidth: 0 }}>
              ...
              <Typography variant="body2" color="text.secondary" mt={0.35}>
                Manage VLS Law Taxation Law registrations and payment data
              </Typography>
            </Box>
          </Stack>
          <Stack direction="row" spacing={1.25} flexWrap="wrap" justifyContent="flex-end">
            <Button
              ...
              sx={{
                borderColor: alpha(VLS_TAXATION_LAW_COLOR, 0.28),
                color: VLS_TAXATION_LAW_COLOR,
                flexShrink: 0,
                '&:hover': { borderColor: VLS_TAXATION_LAW_COLOR, bgcolor: alpha(VLS_TAXATION_LAW_COLOR, 0.04) },
              }}
            >
            ...
            <Button
              ...
              sx={{
                bgcolor: VLS_TAXATION_LAW_COLOR,
                flexShrink: 0,
                '&:hover': { bgcolor: alpha(VLS_TAXATION_LAW_COLOR, 0.88) },
              }}
            >
              Add Registration
            </Button>
        ...
        <Box sx={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
          ...
          >
            {getVlsTaxationLawErrorMessage(
              ...
              'Unable to load Taxation Law registrations.',
            )}
          </Alert>
        </Box>
      ) : (
...
export default VlsTaxationLawSection;
