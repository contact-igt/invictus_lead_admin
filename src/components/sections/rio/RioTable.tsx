...
import { formatRioDateTime } from './rioUtils';

interface RioTableProps {
  rows: RioLead[];
  page: number;
  limit: number;
  total: number;
  ...
  onView: (lead: RioLead) => void;
  onEdit: (lead: RioLead) => void;
  onDelete: (lead: RioLead) => void;
}

interface RioTableRow extends RioLead {
...
const RioTable = ({
  ...
  const tableRows: RioTableRow[] = rows.map((row, index) => ({
    ...row,
    serial_number: (page - 1) * limit + index + 1,
  }));

  const columns: GridColDef<RioTableRow>[] = [
    {
      field: 'serial_number',
      headerName: 'S.No',
      width: 72,
    ...
    },
    {
      ...
      minWidth: 150,
      flex: 0.9,
      renderCell: (params) => params.row.branch ? <Chip label={params.row.branch} size="small" color="secondary" variant="outlined" /> : '-',
    },
    {
      field: 'message',
      headerName: 'Message',
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Typography variant="body2" color="text.secondary">
          {formatRioDateTime(params.row.created_at)}
        </Typography>
      ),
    },
    {
    ...
    return (
      ...
      >
        ...
        <Typography variant="h6" mt={1.5}>
          {hasFilters ? 'No leads match the selected filters.' : 'No Rio leads yet'}
        </Typography>
        {!hasFilters && (
