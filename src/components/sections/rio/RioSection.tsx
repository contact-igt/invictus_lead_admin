...
import {
  ...
  useExportRioLeads,
  useUpdateRioLead,
} from 'hooks/useRioQuery';
import type { RioFormValues } from 'schemas/rioSchema';
import type {
  RioExportFormat,
  RioLead,
  RioListParams,
...
import RioDeleteDialog from './RioDeleteDialog';
import RioFormDrawer, {
  RioDrawerMode,
} from './RioFormDrawer';
...
import {
  ...
  getRioErrorMessage,
  getRioExportErrorMessage,
  getRioExportFallbackName,
  hasRioFilters,
} from './rioUtils';
...
const EMPTY_SUMMARY: RioSummary = {
  total_leads: 0,
  today_leads: 0,
  this_month_leads: 0,
  top_service: null,
...
const RioSection = () => {
  ...
  const isRioModule = resolveClientModuleKey(clientKey) === 'rio';

  const [page, setPage] = useState(1);
  const [limit, setLimit] = useState(20);
  const [searchInput, setSearchInput] = useState('');
  const [debouncedSearch, setDebouncedSearch] = useState('');
  const [service, setService] = useState<RioService | ''>('');
  const [utmSource, setUtmSource] = useState('');
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');

  const [drawerOpen, setDrawerOpen] = useState(false);
  const [drawerMode, setDrawerMode] = useState<RioDrawerMode>('create');
  const [selectedLead, setSelectedLead] = useState<RioLead | null>(null);
  const [deleteLead, setDeleteLead] = useState<RioLead | null>(null);
  ...
  const detailQuery = useRioLead(
    ...
    isRioModule && drawerOpen && drawerMode === 'view',
  );
  const createMutation = useCreateRioLead(clientKey);
  const updateMutation = useUpdateRioLead(clientKey);
  const deleteMutation = useDeleteRioLead(clientKey);
  const exportMutation = useExportRioLeads(clientKey);
  ...
  const openDrawer = (mode: RioDrawerMode, lead: RioLead | null = null) => {
    setSelectedLead(lead);
    setDrawerMode(mode);
    setDrawerOpen(true);
  };

  const handleSubmit = (values: RioFormValues) => {
    const payload = cleanRioPayload(values);
  ...
  const handleExport = async (format: RioExportFormat) => {
    ...
    try {
      ...
      const fileName = extractDownloadFilename(
        ...
        getRioExportFallbackName(format),
      );
      const blob = new Blob([response.data], { type: contentType });

      saveAs(blob, fileName);
      enqueueSnackbar(
        `Rio leads exported as ${format.toUpperCase()} successfully.`,
        { variant: 'success' },
      );
    } catch (error) {
      enqueueSnackbar(getRioExportErrorMessage(error), { variant: 'error' });
    } finally {
  ...
  if (!isRioModule) {
    return (
      <Alert severity="error">
        This page is available only for the Rio client module.
      </Alert>
    );
  }

  const activeFilters = hasRioFilters(params);
    ...
    >
      <Paper
        ...
        sx={{
          ...
          p: { xs: 2.25, md: 3 },
          borderRadius: 3,
          bgcolor: alpha(RIO_COLOR, 0.035),
          borderColor: alpha(RIO_COLOR, 0.18),
        }}
      >
        <Stack
        ...
        >
          ...
          <Stack direction="row" spacing={1.25} flexWrap="wrap" justifyContent="flex-end">
            <Button
              ...
              sx={{
                borderColor: alpha(RIO_COLOR, 0.28),
                color: RIO_COLOR,
                flexShrink: 0,
                '&:hover': {
                  borderColor: RIO_COLOR,
                  bgcolor: alpha(RIO_COLOR, 0.04),
                },
              }}
            >
            ...
            <Button
              ...
              sx={{
                bgcolor: RIO_COLOR,
                flexShrink: 0,
                '&:hover': { bgcolor: alpha(RIO_COLOR, 0.88) },
              }}
            >
              Add Lead
            </Button>
      ...
      >
        ...
        >
          ...
          <TextField
            ...
            sx={{ minWidth: 0 }}
            onChange={(event) => {
              setService(event.target.value as RioService | '');
              setPage(1);
            }}
          >
            <MenuItem value="">All Services</MenuItem>
            {RIO_SERVICES.map((serviceOption) => (
      ...
      {leadsQuery.isError ? (
        <Box sx={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
          ...
          >
            {getRioErrorMessage(leadsQuery.error, 'Unable to load Rio leads.')}
          </Alert>
        </Box>
      ) : (
        <Paper
