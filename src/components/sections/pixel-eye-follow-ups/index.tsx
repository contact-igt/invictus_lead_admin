...
const getFollowUpFilterDate = (lead: LeadRecord): string =>
  ...
  normalizeDateForCompare((lead as any).reminder_scheduled_at);
...
const buildNineAmISO = (dateStr: string): string => {
  // Parse as local midnight then set to 09:00:00
...
const PixelEyeFollowUpsPage: React.FC = () => {
  ...
  const handleSubmitReschedule = async () => {
    ...
    if (rawDate < todayStr) {
      setRescheduleError('Follow-up date cannot be in the past.');
      return;
    }
    ...
    try {
      ...
      setRescheduleError((err as any)?.response?.data?.message || (err as any)?.message || 'Failed to reschedule');
    }
  };
  ...
  const handleSubmitCancel = async () => {
    ...
    const payloadStatus = selectedReason === 'Other' ? notes || undefined : selectedReason || undefined;
    ...
    try {
      ...
      setCancelError((err as any)?.response?.data?.message || (err as any)?.message || 'Failed to cancel follow-up');
    }
  };

  return (
    <div className={`min-h-screen pb-12 transition-colors duration-200 ${mode === 'dark' ? 'bg-[#0A0F0D] text-[#EAF7EE]' : 'bg-slate-50/50 text-slate-800'}`}>
      <div className="px-4 py-6 max-w-7xl mx-auto md:px-6">
        
        {/* --- Header Dashboard Block --- */}
        <div className={`rounded-3xl border p-6 shadow-xl relative overflow-hidden transition-all duration-300 ${
          mode === 'dark' 
            ? 'border-[#183025] bg-[linear-gradient(135deg,#0B1410_0%,#0E1814_50%,#08110D_100%)] shadow-black/30' 
            : 'border-slate-200/80 bg-white shadow-slate-100'
        }`}>
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between relative z-10">
            <div>
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-emerald-500/10 text-[#4ade80]">
                <span className="h-1.5 w-1.5 rounded-full bg-[#4ade80] animate-pulse" />
                PixelEye Follow-ups
              </div>
              <h1 className={`mt-3 text-3xl font-semibold tracking-tight md:text-4xl ${mode === 'dark' ? 'text-white' : 'text-slate-900'}`}>
                Follow-up Queue
              </h1>
              <p className={`mt-2 max-w-2xl text-sm leading-relaxed ${mode === 'dark' ? 'text-[#94A3B8]' : 'text-slate-500'}`}>
                Stay organized and convert more patients. Follow up on active leads categorised dynamically by urgency and lead status.
              </p>
            </div>
            ...
            {sections.map((section) => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-start p-4 rounded-2xl border text-left transition-all duration-200 group relative ${
                    isActive
                      ? mode === 'dark'
                ...
                >
                  ...
                  <div className="mt-4">
                    <span className={`text-xs font-semibold uppercase tracking-wider block ${
                      isActive
                        ? mode === 'dark' ? 'text-[#4ade80]' : 'text-[#156A45]'
                        : mode === 'dark' ? 'text-[#94A3B8]' : 'text-slate-500'
                    }`}>
                      {section.label}
                    </span>
                    <span className={`text-2xl font-bold mt-1 block ${mode === 'dark' ? 'text-white' : 'text-slate-900'}`}>
        ...
        {isLoading ? (
          <div className="mt-8 flex flex-col items-center justify-center py-20">
            ...
            <span className={`mt-4 text-sm font-medium ${mode === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
          ...
          <div className={`mt-8 rounded-3xl border p-12 text-center shadow-sm ${
            ...
            <div className="flex justify-center mb-4">
              <div className={`p-4 rounded-full ${mode === 'dark' ? 'bg-[#12241A]' : 'bg-green-50'}`}>
                <IconifyIcon icon="mdi:calendar-check" sx={{ fontSize: 48, color: '#10B981' }} />
              </div>
            </div>
            <h3 className={`text-xl font-semibold ${mode === 'dark' ? 'text-white' : 'text-slate-900'}`}>
              Zero follow-ups left
            </h3>
            <p className={`mt-2 text-sm max-w-md mx-auto ${mode === 'dark' ? 'text-[#94A3B8]' : 'text-slate-500'}`}>
              All reminders are completed. Leads with scheduled follow-up dates will populate here automatically.
            </p>
          </div>
        ) : (
          <div className="mt-8 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className={`lg:col-span-5 flex flex-col ${mobileView === 'list' ? 'block' : 'hidden'} lg:block`}>
              <div className={`rounded-2xl border p-4 shadow-sm flex flex-col h-[48rem] ${
                mode === 'dark' ? 'border-[#13251D] bg-[#0B1410]' : 'border-slate-200 bg-white'
              }`}>
                {/* Search Bar */}
                <div className="relative mb-4">
                  <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                    <IconifyIcon icon="mdi:magnify" sx={{ fontSize: 18 }} />
                  </div>
                  <input
                    type="text"
                    ...
                    className={`w-full pl-10 pr-4 py-2.5 rounded-xl border text-sm transition-all focus:outline-none focus:ring-1 ${
                      mode === 'dark'
                        ? 'border-[#183025] bg-[#070D0A] text-white focus:border-[#22C55E] focus:ring-[#22C55E]'
                        : 'border-slate-200 bg-slate-50/50 text-slate-900 focus:border-[#156A45] focus:ring-[#156A45]'
                    }`}
                  />
                  {searchQuery && (
                    <button
                    ...
                    activeLeads.map((lead) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full p-4 rounded-xl border text-left transition-all duration-200 flex items-start gap-3 relative overflow-hidden group ${
                            isSelected
                              ? mode === 'dark'
                        ...
                        >
                          ...
                          <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-xs font-bold ${
                            mode === 'dark' ? 'bg-[#152920] text-[#4ade80]' : 'bg-emerald-50 text-emerald-800'
                          }`}>
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-start justify-between gap-2">
                              <span className={`font-semibold text-sm truncate ${mode === 'dark' ? 'text-white' : 'text-slate-900'}`}>
                                {lead.customer_name || 'Unknown customer'}
                              </span>
                              <span className="shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-md" style={{
                                color: accent,
                            ...
                            <div className={`text-xs mt-1 truncate ${mode === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
                              {lead.phone_number || 'N/A'}
                            </div>

                            <div className="flex flex-wrap items-center gap-1.5 mt-3">
                              <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${
                                mode === 'dark' ? 'bg-[#0E1B15] text-[#A7F3D0]' : 'bg-slate-100 text-slate-600'
                              }`}>
                                {lead.follow_up_date ? relativeLabel(lead.daysRelative) : 'No date'}
                              </span>
                              {lead.agent_name && (
                                <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${
                                  mode === 'dark' ? 'bg-[#060D0A] text-slate-400' : 'bg-slate-50 text-slate-500'
                                }`}>
                                  {lead.agent_name}
                                </span>
                              )}
            ...
            <div className={`lg:col-span-7 flex flex-col ${mobileView === 'detail' ? 'block' : 'hidden'} lg:block`}>
              {selectedLead ? (
                <div className={`rounded-2xl border p-6 shadow-sm flex flex-col h-[48rem] justify-between ${
                  mode === 'dark' ? 'border-[#13251D] bg-[#0B1410]' : 'border-slate-200 bg-white'
                }`}>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="lg:hidden flex items-center mb-4">
                      <Button
                        ...
                        sx={{ textTransform: 'none', fontWeight: 600, color: mode === 'dark' ? '#4ade80' : '#156A45' }}
                      >
                        Back to Queue List
                      </Button>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-200/10">
                      <div className="flex items-center gap-4">
                        <div className={`h-16 w-16 rounded-2xl flex items-center justify-center text-xl font-bold shadow-sm ${
                          mode === 'dark' ? 'bg-[#13251D] text-[#DFFFE3]' : 'bg-emerald-50 text-emerald-900'
                        }`}>
                        ...
                        <div>
                          <h2 className={`text-xl font-bold tracking-tight ${mode === 'dark' ? 'text-white' : 'text-slate-900'}`}>
                            {selectedLead.customer_name || 'Unknown Customer'}
                          </h2>
                          <div className={`flex items-center gap-2 mt-1 text-sm ${mode === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
                            <span>{selectedLead.phone_number || 'N/A'}</span>
                            {selectedLead.phone_number && (
                              <Tooltip title={copiedId === selectedLead.id ? 'Copied!' : 'Copy Phone'}>
                                <IconButton 
                                  size="small" 
                                  onClick={() => handleCopyPhone(selectedLead.phone_number, selectedLead.id)}
                                  sx={{ p: 0.5, color: mode === 'dark' ? '#94A3B8' : '#64748B' }}
                                >
                                  <IconifyIcon icon={copiedId === selectedLead.id ? 'mingcute:check-fill' : 'hugeicons:copy-01'} sx={{ fontSize: 15 }} />
                                </IconButton>
                              </Tooltip>
                            )}
                          </div>
                      ...
                      <div className="flex flex-row sm:flex-col items-start gap-2">
                        <Chip
                          label={selectedLead.status || 'N/A'}
                          sx={{
                            backgroundColor: mode === 'dark' ? 'rgba(34, 197, 94, 0.08)' : '#E8F5E9',
                    ...
                    <div className="space-y-4 pt-2">
                      <h3 className={`text-xs font-bold uppercase tracking-widest ${mode === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
                        Lead Details
                      </h3>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className={`p-4 rounded-xl border ${mode === 'dark' ? 'bg-[#070D0A] border-slate-200/10' : 'bg-slate-50/50 border-slate-100'}`}>
                          ...
                          <div className={`text-sm font-semibold mt-1 flex items-center gap-1.5 ${mode === 'dark' ? 'text-white' : 'text-slate-800'}`}>
                            <IconifyIcon icon="mdi:account-tie" sx={{ color: '#10B981', fontSize: 16 }} />
                        ...
                        <div className={`p-4 rounded-xl border ${mode === 'dark' ? 'bg-[#070D0A] border-slate-200/10' : 'bg-slate-50/50 border-slate-100'}`}>
                          ...
                          <div className={`text-sm font-semibold mt-1 flex items-center gap-1.5 ${mode === 'dark' ? 'text-white' : 'text-slate-800'}`}>
                            <IconifyIcon icon="mdi:notebook-edit-outline" sx={{ color: '#F59E0B', fontSize: 16 }} />
                        ...
                        <div className={`p-4 rounded-xl border ${mode === 'dark' ? 'bg-[#070D0A] border-slate-200/10' : 'bg-slate-50/50 border-slate-100'}`}>
                          ...
                          <div className={`text-sm font-semibold mt-1 flex items-center gap-1.5 ${mode === 'dark' ? 'text-white' : 'text-slate-800'}`}>
                            <IconifyIcon icon="mdi:calendar-today" sx={{ color: '#EC4899', fontSize: 16 }} />
                            {selectedLead.follow_up_date ? relativeLabel(selectedLead.daysRelative) : 'N/A'}
                          </div>
                        </div>
                      </div>
                    ...
                    {activeBucket === 'missed' ? (
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 justify-end w-full">
                        <Button
                          ...
                          color="error"
                          onClick={() => handleOpenCancel(selectedLead)}
                          disabled={cancellingLeadId === selectedLead.id || handlingLeadId === selectedLead.id || reschedulingLeadId === selectedLead.id}
                          startIcon={<IconifyIcon icon="mdi:close-circle-outline" />}
                          sx={{
                            borderRadius: '12px',
                            textTransform: 'none',
                        ...
                        <Button
                          variant="outlined"
                          onClick={() => handleOpenReschedule(selectedLead)}
                          disabled={reschedulingLeadId === selectedLead.id || handlingLeadId === selectedLead.id}
                          startIcon={<IconifyIcon icon="mdi:calendar-clock" />}
                          sx={{
                            borderRadius: '12px',
                            textTransform: 'none',
                        ...
                        <Button
                          variant="contained"
                          onClick={() => handleMarkFollowedUp(selectedLead.id)}
                          disabled={handlingLeadId === selectedLead.id || reschedulingLeadId === selectedLead.id || cancellingLeadId === selectedLead.id}
                          startIcon={handlingLeadId === selectedLead.id ? <CircularProgress size={14} sx={{ color: '#fff' }} /> : <IconifyIcon icon="mdi:check-circle" />}
                          sx={{
                            borderRadius: '12px',
                            textTransform: 'none',
                            fontWeight: 600,
                      ...
                      <div className={`p-4 rounded-xl border text-center ${
                        mode === 'dark' ? 'bg-[#070D0A] border-[#15271E] text-[#DFFFE3]' : 'bg-slate-50 border-slate-200 text-slate-500'
                      }`}>
                        Follow-up state is <strong>{selectedLead.followup_state}</strong>. No actions available.
                      </div>
                    )}
                  </div>
                ...
                <div className={`rounded-2xl border p-6 flex flex-col justify-center items-center h-[48rem] text-center shadow-sm ${
                  mode === 'dark' ? 'border-[#13251D] bg-[#0B1410] text-slate-400' : 'border-slate-200 bg-white text-slate-500'
                }`}>
                  <div className={`p-4 rounded-full mb-4 ${mode === 'dark' ? 'bg-[#12241A]' : 'bg-green-50'}`}>
                    <IconifyIcon icon="mdi:calendar-multiple-check" sx={{ fontSize: 48, color: '#10B981' }} />
                  </div>
                  <h3 className={`text-lg font-semibold ${mode === 'dark' ? 'text-white' : 'text-slate-800'}`}>
                    No patient lead selected
                  </h3>
                  <p className="mt-2 text-sm max-w-sm">
                    Select a follow-up lead from the queue pane to view details, copy phone credentials, and take direct actions.
                  </p>
                </div>
              )}
            </div>
      ...
      <Drawer
        ...
        PaperProps={{
          sx: {
            ...
            boxShadow: mode === 'dark' ? '-10px 0 35px rgba(0,0,0,0.5)' : '-10px 0 35px rgba(0,0,0,0.05)',
      ...
      >
        ...
        <div className={`flex items-center justify-between px-6 py-5 border-b shrink-0 ${
          mode === 'dark' ? 'border-[#14241C]' : 'border-slate-100'
        }`}>
          <div className="flex items-center gap-2.5">
            <div className={`p-2 rounded-lg ${mode === 'dark' ? 'bg-[#12241A] text-[#10B981]' : 'bg-emerald-50 text-emerald-800'}`}>
              <IconifyIcon icon="mdi:calendar-clock" sx={{ fontSize: 20 }} />
            </div>
            <div>
              <h3 className={`text-lg font-bold tracking-tight ${mode === 'dark' ? 'text-white' : 'text-slate-900'}`}>
                Reschedule Follow-up
              </h3>
              <p className={`text-xs ${mode === 'dark' ? 'text-slate-400' : 'text-slate-500'} mt-0.5`}>
                Schedule a future callback for this lead
              </p>
            </div>
          </div>
          <IconButton onClick={handleCloseReschedule} size="small" sx={{ color: mode === 'dark' ? '#94A3B8' : '#64748B' }}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <div className={`p-4 rounded-xl border ${
            mode === 'dark' ? 'bg-[#060B08] border-[#14241C]' : 'bg-slate-50 border-slate-100'
          }`}>
            <div className={`text-sm font-semibold ${mode === 'dark' ? 'text-white' : 'text-slate-900'}`}>
              {rescheduleLead?.customer_name || 'Unknown customer'}
            </div>
            <div className={`mt-2 text-xs ${mode === 'dark' ? 'text-slate-400' : 'text-slate-500'} space-y-1.5`}>
        ...
        {/* Footer */}
        <div className={`px-6 py-5 border-t shrink-0 flex items-center justify-end gap-3 ${
          mode === 'dark' ? 'border-[#14241C]' : 'border-slate-100'
        }`}>
          <Button
      ...
      >
        {/* Header */}
        <div className={`flex items-center justify-between px-6 py-5 border-b shrink-0 ${
          mode === 'dark' ? 'border-[#281414]' : 'border-slate-100'
        }`}>
          <div className="flex items-center gap-2.5">
            <div className={`p-2 rounded-lg ${mode === 'dark' ? 'bg-[#281414] text-[#DC2626]' : 'bg-red-50 text-red-800'}`}>
            ...
            <div>
              ...
              <p className={`text-xs ${mode === 'dark' ? 'text-rose-300/80' : 'text-slate-500'} mt-0.5`}>
                Archive or close this reminder record
              </p>
            </div>
          </div>
          <IconButton onClick={handleCloseCancel} size="small" sx={{ color: mode === 'dark' ? '#FCA5A5' : '#64748B' }}>
            <IconifyIcon icon="mdi:close" sx={{ fontSize: 20 }} />
          </IconButton>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          {/* Customer Summary Card */}
          <div className={`p-4 rounded-xl border ${
            mode === 'dark' ? 'bg-[#0A0505] border-[#281414]' : 'bg-rose-50/50 border-rose-100'
          }`}>
            <div className={`text-sm font-semibold ${mode === 'dark' ? 'text-white' : 'text-slate-900'}`}>
              {cancelLead?.customer_name || 'Unknown customer'}
            </div>
            <div className={`mt-2 text-xs ${mode === 'dark' ? 'text-rose-300' : 'text-rose-700'} space-y-1.5`}>
              <div className="flex items-center gap-2">
