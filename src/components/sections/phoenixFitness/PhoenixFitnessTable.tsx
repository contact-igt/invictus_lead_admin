...
interface PhoenixFitnessTableProps {
  rows: PhoenixFitnessLead[];
  page: number;
  limit: number;
  total: number;
  isLoading: boolean;
  ...
  onView: (lead: PhoenixFitnessLead) => void;
  onEdit: (lead: PhoenixFitnessLead) => void;
  onDelete: (lead: PhoenixFitnessLead) => void;
}

interface PhoenixFitnessTableRow extends PhoenixFitnessLead {
  serial_number: number;
}

const PhoenixFitnessTable = ({
  rows,
  page,
  limit,
  total,
  ...
  onEdit,
  onDelete,
}: PhoenixFitnessTableProps) => {
  const tableRows: PhoenixFitnessTableRow[] = rows.map((row, index) => ({
    ...row,
    serial_number: (page - 1) * limit + index + 1,
  }));

  const columns: GridColDef<PhoenixFitnessTableRow>[] = [
    {
      field: 'serial_number',
      headerName: 'S.No',
      width: 72,
    ...
    },
    {
      ...
      flex: 1,
      renderCell: (params) =>
        params.row.branch ? <Chip label={params.row.branch} size="small" variant="outlined" /> : '-',
    },
    {
      field: 'ip_address',
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Typography variant="body2" color="text.secondary">
          {formatPhoenixFitnessDateTime(params.row.created_at)}
        </Typography>
      ),
    },
    {
    ...
    return (
      ...
      >
        ...
        <Typography variant="h6" mt={1.5}>
          {hasFilters ? 'No leads match the selected filters.' : 'No Phoenix Fitness leads yet'}
        </Typography>
        {!hasFilters && (
...
export default PhoenixFitnessTable;
