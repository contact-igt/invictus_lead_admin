...
import {
  PhoenixFitnessFormValues,
  phoenixFitnessInitialValues,
  phoenixFitnessSchema,
} from 'schemas/phoenixFitnessSchema';
import type { PhoenixFitnessLead } from 'types/phoenixFitness';
import { PHOENIX_FITNESS_BRANCHES, formatPhoenixFitnessDateTime } from './phoenixFitnessUtils';

export type PhoenixFitnessDrawerMode = 'create' | 'edit' | 'view';

interface PhoenixFitnessFormDrawerProps {
  open: boolean;
  mode: PhoenixFitnessDrawerMode;
  lead: PhoenixFitnessLead | null;
  isLoading: boolean;
  onClose: () => void;
  onSubmit: (values: PhoenixFitnessFormValues) => void;
}

const PhoenixFitnessFormDrawer = ({
  open,
  mode,
  lead,
  isLoading,
  onClose,
  onSubmit,
}: PhoenixFitnessFormDrawerProps) => {
  const isView = mode === 'view';
  const isEdit = mode === 'edit';

  const formik = useFormik<PhoenixFitnessFormValues>({
    initialValues: lead
      ? {
          name: lead.name ?? '',
      ...
      : phoenixFitnessInitialValues,
    validationSchema: phoenixFitnessSchema,
  ...
  const field = (
    name: keyof PhoenixFitnessFormValues,
    label: string,
    required = false,
    fullWidth = false,
  ) => (
    <Grid item xs={12} sm={fullWidth ? 12 : 6}>
      {name === 'branch' ? (
        ...
        >
          ...
          {PHOENIX_FITNESS_BRANCHES.map((option) => (
  ...
  const title = isView
    ? 'Phoenix Fitness Lead'
    : isEdit
      ? 'Edit Phoenix Fitness Lead'
      : 'Add Phoenix Fitness Lead';
        ...
        <Box sx={{ flex: 1, overflowY: 'auto', px: { xs: 2, sm: 3 }, py: 3 }}>
          {isView ? (
            <Stack spacing={3}>
              ...
              >
                {viewItem('Created At', formatPhoenixFitnessDateTime(lead?.created_at))}
                {viewItem('Updated At', formatPhoenixFitnessDateTime(lead?.updated_at))}
              </Box>
            </Stack>
          ) : (
...
export default PhoenixFitnessFormDrawer;
