...
const RightPanel: React.FC<RightPanelProps> = ({ actions = {} }) => {
  ...
  const { highPriorityCount = 0, highPriorityLeads = [] } = actions;
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div
            className={`text-xs truncate ${mode === 'dark' ? 'text-[#94A3B8]' : 'text-gray-500'}`}
          >
            {lead.phone_number}
          </div>
        </div>
  ...
  return (
    <div className="sticky top-24 space-y-4">
      ...
      <div className={`pb-3 border-b ${mode === 'dark' ? 'border-[#1E2E25]' : 'border-gray-200'}`}>
        <div className={`text-lg font-bold ${mode === 'dark' ? 'text-white' : 'text-gray-900'}`}>
          Actions
        </div>
      ...
      <div>
        <div
          className={`mb-2 pb-2 border-b ${mode === 'dark' ? 'border-[#0F1B16]' : 'border-gray-200'}`}
        >
          ...
          <div className={`text-xs mt-0.5 ${mode === 'dark' ? 'text-[#94A3B8]' : 'text-gray-600'}`}>
            ...
            {remainingCount > 0 && (
              <span
                className={`font-semibold ${mode === 'dark' ? 'text-[#16A34A]' : 'text-green-600'}`}
              >
                {' '}
                (+{remainingCount} more)
              </span>
        ...
        <div className="space-y-1 max-h-[calc(100vh-320px)] overflow-y-auto custom-scrollbar">
          {displayedLeads.length > 0 ? (
            displayedLeads.map((lead) => <LeadItem key={lead.id} lead={lead} />)
          ) : (
            <div
              className={`text-xs py-4 text-center ${mode === 'dark' ? 'text-[#94A3B8]' : 'text-gray-500'}`}
            >
        ...
        {displayedLeads.length > 0 && (
          <div
            className={`mt-3 pt-3 border-t ${mode === 'dark' ? 'border-[#0F1B16]' : 'border-gray-200'}`}
          >
            <button
