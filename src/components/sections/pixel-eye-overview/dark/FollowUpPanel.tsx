...
const LeadRow = ({
  ...
  <div
    className={`flex items-center gap-3 p-3 rounded-md transition-colors h-[68px] ${
  ...
  >
    ...
    <div className="flex-1 min-w-0">
      ...
      <div className={`text-xs truncate ${mode === 'dark' ? 'text-[#94A3B8]' : 'text-gray-600'}`}>
        {lead.phone_number} • {lead.agent_name}
      </div>
      <div className="text-xs font-semibold mt-1" style={{ color: accent }}>
        {relativeLabel(lead.daysRelative)} — {lead.follow_up_date}
      </div>
    </div>
  </div>
);

const FollowUpPanel: React.FC<{ followUps: FollowUpMetrics; loading?: boolean }> = ({
  followUps,
  loading = false,
}) => {
