...
import type { VlsBusinessLawRegistration } from 'types/vlsBusinessLaw';
import {
  formatCaptured,
  formatVlsBusinessLawAmount,
  formatVlsBusinessLawDate,
  formatVlsBusinessLawDateTime,
  VLS_BUSINESS_LAW_PAYMENT_STATUS_OPTIONS,
} from './vlsBusinessLawUtils';

interface VlsBusinessLawTableProps {
  rows: VlsBusinessLawRegistration[];
  page: number;
  limit: number;
  total: number;
  isLoading: boolean;
  ...
  onView: (registration: VlsBusinessLawRegistration) => void;
  onEdit: (registration: VlsBusinessLawRegistration) => void;
  onDelete: (registration: VlsBusinessLawRegistration) => void;
  onStatusChange?: (id: number, paymentStatus: string) => void;
}

interface TableRow extends VlsBusinessLawRegistration {
...
const InlinePaymentStatusCell = ({
  ...
  return (
    <>
      ...
      >
        {VLS_BUSINESS_LAW_PAYMENT_STATUS_OPTIONS.map((option) => (
          <MenuItem
            key={option}
...
const VlsBusinessLawTable = ({
  ...
  const columns: GridColDef<TableRow>[] = [
    ...
    },
    {
      ...
      align: 'right',
      headerAlign: 'right',
      renderCell: (params) => <Typography variant="body2" fontWeight={650}>{formatVlsBusinessLawAmount(params.row.amount)}</Typography>,
    },
    {
      ...
      minWidth: 195,
      flex: 1.5,
      renderCell: (params) => <Typography variant="body2" color="text.secondary">{formatVlsBusinessLawDate(params.row.registered_date)}</Typography>,
    },
    {
      field: 'programm_date',
      ...
      minWidth: 150,
      flex: 1.1,
      renderCell: (params) => <Typography variant="body2" color="text.secondary">{formatVlsBusinessLawDate(params.row.programm_date)}</Typography>,
    },
    {
    ...
    },
    {
      ...
      minWidth: 185,
      flex: 1,
      renderCell: (params) => <Typography variant="body2" color="text.secondary">{formatVlsBusinessLawDateTime(params.row.created_at)}</Typography>,
    },
    {
      field: 'actions',
      headerName: 'Actions',
    ...
    return (
      <Paper variant="outlined" sx={{ width: '100%', minWidth: 0, py: 8, px: 3, textAlign: 'center', borderRadius: 3 }}>
        ...
        <Typography variant="h6" mt={1.5}>
          {hasFilters ? 'No registrations match the selected filters.' : 'No Business Law registrations yet'}
        </Typography>
...
export default VlsBusinessLawTable;
