...
export const VLS_BUSINESS_LAW_COLOR = '#1F6B40';
export const VLS_BUSINESS_LAW_PAYMENT_STATUS_OPTIONS = [
  'paid',
  'waitlist',
  'attempted',
  'failed',
...
export const formatVlsBusinessLawDateTime = (value?: string | null): string => {
  return formatVlsDateTime(value);
};

export const formatVlsBusinessLawDate = (value?: string | null): string => {
  return formatVlsDateTime(value);
};

export const formatVlsBusinessLawAmount = (value?: string | number | null): string => {
...
export const cleanVlsBusinessLawPayload = (
  values: VlsBusinessLawFormValues,
): CreateVlsBusinessLawPayload => ({
...
export const hasVlsBusinessLawFilters = (params: VlsBusinessLawListParams): boolean =>
  Boolean(
    params.search ||
...
export const getVlsBusinessLawErrorStatus = (error: unknown): number | undefined => {
  if (!axios.isAxiosError(error)) return undefined;
  return (error as AxiosError<BackendErrorPayload>).response?.status;
};

export const getVlsBusinessLawErrorMessage = (
  error: unknown,
  ...
  if (status === 404) return 'The requested Business Law registration was not found.';
  if (status === 429) return 'Too many requests. Please try again shortly.';
  if (status && status >= 500) return 'Something went wrong while processing the request.';
  return backendMessage || fallback;
};

export const getVlsBusinessLawExportErrorMessage = (error: unknown): string => {
  ...
  if (status === 429) return 'Too many requests. Please try again shortly.';
  if (status && status >= 500) return 'Unable to export Business Law registrations.';
  return getVlsBusinessLawErrorMessage(error, 'Unable to export Business Law registrations.');
};
...
export const getVlsBusinessLawExportFallbackName = (format: 'csv' | 'pdf'): string => {
  const date = dayjs().format('YYYY-MM-DD');
  return `vls-business-law-registrations-${date}.${format}`;
};
