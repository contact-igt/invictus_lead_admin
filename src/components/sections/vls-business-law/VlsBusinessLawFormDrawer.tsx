...
import {
  VlsBusinessLawFormValues,
  vlsBusinessLawInitialValues,
  vlsBusinessLawSchema,
} from 'schemas/vlsBusinessLawSchema';
...
export type VlsBusinessLawDrawerMode = 'create' | 'edit' | 'view';

interface VlsBusinessLawFormDrawerProps {
  open: boolean;
  mode: VlsBusinessLawDrawerMode;
  registration: VlsBusinessLawRegistration | null;
  isLoading: boolean;
  onClose: () => void;
  onSubmit: (values: VlsBusinessLawFormValues) => void;
}

const toDateInput = (value?: string | null) => (value ? String(value).slice(0, 10) : '');

const VlsBusinessLawFormDrawer = ({
  open,
  mode,
  registration,
  isLoading,
  onClose,
  onSubmit,
}: VlsBusinessLawFormDrawerProps) => {
  const isView = mode === 'view';
  const isEdit = mode === 'edit';

  const formik = useFormik<VlsBusinessLawFormValues>({
    initialValues: registration
      ...
      : vlsBusinessLawInitialValues,
    validationSchema: vlsBusinessLawSchema,
  ...
  const renderTextField = (
    name: keyof VlsBusinessLawFormValues,
    label: string,
    type: string = 'text',
    required = false,
  ) => (
  ...
  const title = isView
    ...
    : isEdit
      ? 'Edit Business Law Registration'
      : 'Add Business Law Registration';
              ...
              <Typography variant="body2" color="text.secondary" mt={0.5}>
                {isView
                  ...
                  : isEdit
                    ...
                    : 'Create a new Business Law registration'}
              </Typography>
            </Box>
...
export default VlsBusinessLawFormDrawer;
