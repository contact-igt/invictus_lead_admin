...
import {
  useCreateVlsBusinessLawRegistration,
  useDeleteVlsBusinessLawRegistration,
  useUpdateVlsBusinessLawRegistration,
  useVlsBusinessLawRegistration,
  useVlsBusinessLawRegistrations,
  useVlsBusinessLawSummary,
} from 'hooks/useVlsBusinessLawQuery';
import { useAuth } from 'redux/selectors/auth/authSelector';
import type { VlsBusinessLawFormValues } from 'schemas/vlsBusinessLawSchema';
import { exportVlsBusinessLawRegistrations } from 'services/vlsBusinessLaw.service';
import type {
  VlsBusinessLawExportFormat,
  VlsBusinessLawListParams,
  VlsBusinessLawRegistration,
  VlsBusinessLawSummary,
} from 'types/vlsBusinessLaw';
import { resolveClientModuleKey } from 'utils/clientModuleResolver';
import VlsBusinessLawDeleteDialog from './VlsBusinessLawDeleteDialog';
import VlsBusinessLawFormDrawer, {
  VlsBusinessLawDrawerMode,
} from './VlsBusinessLawFormDrawer';
import VlsBusinessLawTable from './VlsBusinessLawTable';
import {
  VLS_BUSINESS_LAW_COLOR,
  VLS_BUSINESS_LAW_PAYMENT_STATUS_OPTIONS,
  cleanVlsBusinessLawPayload,
  extractDownloadFilename,
  getVlsBusinessLawErrorMessage,
  getVlsBusinessLawExportErrorMessage,
  getVlsBusinessLawExportFallbackName,
  hasVlsBusinessLawFilters,
} from './vlsBusinessLawUtils';
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={1.5}>
        ...
        <Box
          sx={{
            ...
            display: 'grid',
            placeItems: 'center',
            bgcolor: alpha(VLS_BUSINESS_LAW_COLOR, 0.1),
            color: VLS_BUSINESS_LAW_COLOR,
            flexShrink: 0,
          }}
        >
...
const EMPTY_SUMMARY: VlsBusinessLawSummary = {
...
const VlsBusinessLawSection = () => {
  ...
  const [drawerMode, setDrawerMode] = useState<VlsBusinessLawDrawerMode>('create');
  const [selectedRegistration, setSelectedRegistration] =
    useState<VlsBusinessLawRegistration | null>(null);
  ...
  const params: VlsBusinessLawListParams = {
    page,
    limit,
  ...
  const registrationsQuery = useVlsBusinessLawRegistrations(clientKey, params, isVlsModule);
  const summaryQuery = useVlsBusinessLawSummary(clientKey, params, isVlsModule);
  const detailQuery = useVlsBusinessLawRegistration(
    clientKey,
    selectedRegistration?.id ?? null,
    isVlsModule && drawerOpen && drawerMode === 'view',
  );
  const createMutation = useCreateVlsBusinessLawRegistration(clientKey);
  const updateMutation = useUpdateVlsBusinessLawRegistration(clientKey);
  const deleteMutation = useDeleteVlsBusinessLawRegistration(clientKey);
  ...
  const openDrawer = (
    mode: VlsBusinessLawDrawerMode,
    registration: VlsBusinessLawRegistration | null = null,
  ) => {
  ...
  const handleSubmit = (values: VlsBusinessLawFormValues) => {
    const payload = cleanVlsBusinessLawPayload(values);
  ...
  const handleExport = async (format: VlsBusinessLawExportFormat) => {
    handleCloseExportMenu();
    setIsExporting(true);
    try {
      const response = await exportVlsBusinessLawRegistrations(
        format,
        params,
        superAdminClientKey,
      );
      ...
      const fileName = extractDownloadFilename(
        ...
        getVlsBusinessLawExportFallbackName(format),
      );
      saveAs(new Blob([response.data], { type: contentType }), fileName);
      enqueueSnackbar(
        `Business Law registrations exported as ${format.toUpperCase()} successfully.`,
        { variant: 'success' },
      );
    } catch (error) {
      enqueueSnackbar(getVlsBusinessLawExportErrorMessage(error), { variant: 'error' });
    } finally {
  ...
  const hasFilters = hasVlsBusinessLawFilters(params);
    ...
    >
      <Paper
        ...
        sx={{
          ...
          p: { xs: 2.25, md: 3 },
          borderRadius: 3,
          bgcolor: alpha(VLS_BUSINESS_LAW_COLOR, 0.035),
          borderColor: alpha(VLS_BUSINESS_LAW_COLOR, 0.18),
        }}
      >
        <Stack
        ...
        >
          <Stack direction="row" alignItems="center" spacing={1.75} sx={{ minWidth: 0, flex: 1 }}>
            <Box
              sx={{
                ...
                placeItems: 'center',
                color: 'common.white',
                bgcolor: VLS_BUSINESS_LAW_COLOR,
              }}
            >
              <IconifyIcon icon="mingcute:bank-line" width={24} />
            </Box>
            <Box sx={{ minWidth: 0 }}>
              <Typography
                variant="h4"
              ...
              <Typography variant="body2" color="text.secondary" mt={0.35}>
                Manage VLS Law Business Law registrations and payment data
              </Typography>
            </Box>
          </Stack>
          <Stack direction="row" spacing={1.25} flexWrap="wrap" justifyContent="flex-end">
            <Button
              ...
              sx={{
                borderColor: alpha(VLS_BUSINESS_LAW_COLOR, 0.28),
                color: VLS_BUSINESS_LAW_COLOR,
                flexShrink: 0,
                '&:hover': { borderColor: VLS_BUSINESS_LAW_COLOR, bgcolor: alpha(VLS_BUSINESS_LAW_COLOR, 0.04) },
              }}
            >
            ...
            <Button
              ...
              sx={{
                bgcolor: VLS_BUSINESS_LAW_COLOR,
                flexShrink: 0,
                '&:hover': { bgcolor: alpha(VLS_BUSINESS_LAW_COLOR, 0.88) },
              }}
            >
              Add Registration
            </Button>
      ...
      >
        ...
        >
          ...
          >
            ...
            {VLS_BUSINESS_LAW_PAYMENT_STATUS_OPTIONS.map((status) => (
        ...
        <Box sx={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
          ...
          >
            {getVlsBusinessLawErrorMessage(
              ...
              'Unable to load Business Law registrations.',
            )}
          </Alert>
        </Box>
      ) : (
...
export default VlsBusinessLawSection;
