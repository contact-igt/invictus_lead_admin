...
import { formatShantiEyeTechDateTime } from './shantiEyeTechUtils';

interface ShantiEyeTechTableProps {
  rows: ShantiEyeTechLead[];
  page: number;
  limit: number;
  total: number;
  isLoading: boolean;
  ...
  onView: (lead: ShantiEyeTechLead) => void;
  onEdit: (lead: ShantiEyeTechLead) => void;
  onDelete: (lead: ShantiEyeTechLead) => void;
}

interface ShantiEyeTechTableRow extends ShantiEyeTechLead {
  serial_number: number;
}

const ShantiEyeTechTable = ({
  rows,
  page,
  limit,
  total,
  ...
  onEdit,
  onDelete,
}: ShantiEyeTechTableProps) => {
  const tableRows: ShantiEyeTechTableRow[] = rows.map((row, index) => ({
    ...row,
    serial_number: (page - 1) * limit + index + 1,
  }));

  const columns: GridColDef<ShantiEyeTechTableRow>[] = [
    {
      field: 'serial_number',
      headerName: 'S.No',
      width: 72,
    ...
    },
    {
      ...
      minWidth: 220,
      flex: 1.2,
      renderCell: (params) => <Typography variant="body2" noWrap title={params.row.message || ''}>{params.row.message || '-'}</Typography>,
    },
    {
      field: 'ip_address',
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Typography variant="body2" color="text.secondary">
          {formatShantiEyeTechDateTime(params.row.created_at)}
        </Typography>
      ),
    },
    {
    ...
    return (
      ...
      >
        ...
        <Typography variant="h6" mt={1.5}>
          {hasFilters ? 'No leads match the selected filters.' : 'No Shanti Eye Tech leads yet'}
        </Typography>
        {!hasFilters && (
...
export default ShantiEyeTechTable;
