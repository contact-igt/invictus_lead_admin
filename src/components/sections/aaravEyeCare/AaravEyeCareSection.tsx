...
import {
  useAaravEyeCareLead,
  useAaravEyeCareLeads,
  useAaravEyeCareSummary,
  useCreateAaravEyeCareLead,
  useDeleteAaravEyeCareLead,
  useUpdateAaravEyeCareLead,
} from 'hooks/useAaravEyeCareQuery';
import { useAuth } from 'redux/selectors/auth/authSelector';
import type { AaravEyeCareFormValues } from 'schemas/aaravEyeCareSchema';
import { exportAaravEyeCareLeads } from 'services/aaravEyeCare.service';
import type {
...
import AaravEyeCareDeleteDialog from './AaravEyeCareDeleteDialog';
import AaravEyeCareFormDrawer, {
  AaravEyeCareDrawerMode,
} from './AaravEyeCareFormDrawer';
import AaravEyeCareTable from './AaravEyeCareTable';
import {
  AARAV_EYE_CARE_COLOR,
  AARAV_EYE_CARE_SERVICES,
  cleanAaravPayload,
  extractDownloadFilename,
  getAaravErrorMessage,
  getAaravExportErrorMessage,
  getAaravExportFallbackName,
  hasAaravFilters,
} from './aaravEyeCareUtils';
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={1.5}>
        ...
        <Box
          sx={{
            ...
            display: 'grid',
            placeItems: 'center',
            bgcolor: alpha(AARAV_EYE_CARE_COLOR, 0.1),
            color: AARAV_EYE_CARE_COLOR,
            flexShrink: 0,
          }}
        >
...
const EMPTY_SUMMARY: AaravEyeCareSummary = {
  total_leads: 0,
  today_leads: 0,
  this_month_leads: 0,
  top_service: null,
...
const AaravEyeCareSection = () => {
  const { clientKey } = useParams<{ clientKey: string }>();
  const { user } = useAuth();
  const { enqueueSnackbar } = useSnackbar();
  const isAaravModule = resolveClientModuleKey(clientKey) === 'aarav_eye_care';
  ...
  const [drawerMode, setDrawerMode] = useState<AaravEyeCareDrawerMode>('create');
  const [selectedLead, setSelectedLead] = useState<AaravEyeCareLead | null>(null);
  const [deleteLead, setDeleteLead] = useState<AaravEyeCareLead | null>(null);
  ...
  const params: AaravEyeCareListParams = {
    page,
    limit,
  ...
  const detailQuery = useAaravEyeCareLead(
    ...
    isAaravModule && drawerOpen && drawerMode === 'view',
  );
  const createMutation = useCreateAaravEyeCareLead(clientKey);
  const updateMutation = useUpdateAaravEyeCareLead(clientKey);
  const deleteMutation = useDeleteAaravEyeCareLead(clientKey);
  ...
  const openDrawer = (mode: AaravEyeCareDrawerMode, lead: AaravEyeCareLead | null = null) => {
    setSelectedLead(lead);
    setDrawerMode(mode);
    setDrawerOpen(true);
  };

  const handleSubmit = (values: AaravEyeCareFormValues) => {
    const payload = cleanAaravPayload(values);
  ...
  const handleExport = async (format: AaravEyeCareExportFormat) => {
    ...
    try {
      ...
      const response = await exportAaravEyeCareLeads(format, exportParams, superAdminClientKey);
      const contentType = response.headers['content-type'] || (format === 'pdf' ? 'application/pdf' : 'text/csv;charset=utf-8');
      const fileName = extractDownloadFilename(
        response.headers['content-disposition'],
        getAaravExportFallbackName(format),
      );
      const blob = new Blob([response.data], { type: contentType });

      saveAs(blob, fileName);
      enqueueSnackbar(
        `Aarav Eye Care leads exported as ${format.toUpperCase()} successfully.`,
        { variant: 'success' },
      );
    } catch (error) {
      enqueueSnackbar(getAaravExportErrorMessage(error), { variant: 'error' });
    } finally {
  ...
  if (!isAaravModule) {
    return (
      <Alert severity="error">
        This page is available only for the Aarav Eye Care client module.
      </Alert>
    );
  }

  const activeFilters = hasAaravFilters(params);
    ...
    >
      <Paper
        ...
        sx={{
          ...
          p: { xs: 2.25, md: 3 },
          borderRadius: 3,
          bgcolor: alpha(AARAV_EYE_CARE_COLOR, 0.035),
          borderColor: alpha(AARAV_EYE_CARE_COLOR, 0.18),
        }}
      >
        <Stack
        ...
        >
          ...
          <Stack direction="row" spacing={1.25} flexWrap="wrap" justifyContent="flex-end">
            <Button
              ...
              sx={{
                borderColor: alpha(AARAV_EYE_CARE_COLOR, 0.28),
                color: AARAV_EYE_CARE_COLOR,
                flexShrink: 0,
                '&:hover': {
                  borderColor: AARAV_EYE_CARE_COLOR,
                  bgcolor: alpha(AARAV_EYE_CARE_COLOR, 0.04),
                },
              }}
            >
            ...
            <Button
              ...
              sx={{
                bgcolor: AARAV_EYE_CARE_COLOR,
                flexShrink: 0,
                '&:hover': { bgcolor: alpha(AARAV_EYE_CARE_COLOR, 0.88) },
              }}
            >
              Add Lead
            </Button>
      ...
      >
        ...
        >
          ...
          >
            ...
            {AARAV_EYE_CARE_SERVICES.map((serviceOption) => (
      ...
      {leadsQuery.isError ? (
        <Box sx={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
          ...
          >
            {getAaravErrorMessage(leadsQuery.error, 'Unable to load Aarav Eye Care leads.')}
          </Alert>
        </Box>
      ) : (
        <Paper
...
export default AaravEyeCareSection;
