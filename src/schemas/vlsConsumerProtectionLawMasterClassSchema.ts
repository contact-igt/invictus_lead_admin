...
export interface VlsConsumerProtectionLawMasterClassFormValues {
  name: string;
  mobile: string;
  email: string;
  city: string;
...
export const vlsConsumerProtectionLawMasterClassInitialValues: VlsConsumerProtectionLawMasterClassFormValues = {
  name: '',
  mobile: '',
  email: '',
  city: '',
...
export const vlsConsumerProtectionLawMasterClassSchema = Yup.object({
  name: Yup.string()
    .trim()
  ...
  city: Yup.string().trim().max(100, 'City must be 100 characters or fewer'),
  profession: Yup.string().trim().max(100, 'Profession must be 100 characters or fewer'),
  amount: Yup.number()
